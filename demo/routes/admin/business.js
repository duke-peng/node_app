...
router.get('/',function(req,res){
    ...
    MongoClient.connect(DbUrl,function(err,db){
        ...
        var result=db.collection('artice').find();
        ...
        result.toArray(function(error,data){
            ...
            res.render('admin/business/index',{
...
router.post('/doAdd',function(req,res){
    ...
    form.parse(req, function(err, fields, files) {
        ...
        MongoClient.connect(DbUrl,function(err,db){
            ...
            db.collection('artice').insertOne({
                title,
                author,
                content,
                addtime,
...
router.post("/find",function(req,res){
    MongoClient.connect(DbUrl,function(err,db){
        ...
        var result=db.collection('artice').find({"_id":new ObjectID(req.body._id)});
        result.toArray(function(error,data){
            if(data.length>0){
...
router.post('/doEdit',function(req,res) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        MongoClient.connect(DbUrl, function (err, db) {
            ...
            db.collection('artice').updateOne({"_id": new ObjectID(_id)}, {
                $set: setData
            }, function (error) {
...
router.get('/delete',function(req,res){
    ...
    MongoClient.connect(DbUrl, function (err, db) {
        ...
        db.collection('artice').deleteOne({"_id": new ObjectID(id)}, {
            id:false
        })
        res.redirect('./');
